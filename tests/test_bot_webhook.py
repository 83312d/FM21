...
def test_webhook_secret_required_without_open_access(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(bot_main, "TELEGRAM_WEBHOOK_SECRET", "")
    monkeypatch.setattr(bot_main, "BOT_OPEN_ACCESS", False)

    with pytest.raises(HTTPException) as exc:
        bot_main._validate_webhook_secret(None)
...
def test_webhook_secret_skipped_in_dev_open_access(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(bot_main, "TELEGRAM_WEBHOOK_SECRET", "")
    monkeypatch.setattr(bot_main, "BOT_OPEN_ACCESS", True)
...
def test_webhook_secret_rejects_wrong_token(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(bot_main, "TELEGRAM_WEBHOOK_SECRET", "expected-secret")
    monkeypatch.setattr(bot_main, "BOT_OPEN_ACCESS", False)

    with pytest.raises(HTTPException) as exc:
        bot_main._validate_webhook_secret("wrong")
